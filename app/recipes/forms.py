...
# ... (Helper function et IngredientForm restent inchangés)
...
class RecipeForm(FlaskForm):
    ...
    # ### CORRECTION : Le nom du champ est maintenant 'product_id' ###
    # Il correspond exactement au nom de la colonne dans le modèle.
    product_id = SelectField('Produit Fini Associé', coerce=int, validators=[Optional()])
    ...
    def __init__(self, *args, **kwargs):
        ...
        # ### CORRECTION : On peuple les choix pour le champ 'product_id' ###
        self.product_id.choices = [(p.id, p.name) for p in query.order_by(Product.name).all()]
        self.product_id.choices.insert(0, (0, '-- Aucun --'))