...
# Suppression de l'import pytz, qui n'est plus nécessaire car le bug était dans le template.
...
def list_purchases():
    ...
    stats = {
        ...
        'pending_approval': Purchase.query.filter(Purchase.status.in_([PurchaseStatus.DRAFT, PurchaseStatus.REQUESTED])).count(),
...
def new_purchase():
    ...
    if form.validate_on_submit():
        # ### DEBUT DE LA MODIFICATION ###
        # Le code de localisation de fuseau horaire a été retiré.
        # Nous utilisons maintenant directement la donnée du formulaire, ce qui est la méthode correcte.
        purchase = Purchase(
            ...
            requested_date=form.requested_date.data, # <-- Direct et propre
        ...
        if items_added == 0:
            ...
            return render_template('purchases/new_purchase.html', form=form, title='Nouveau Bon d\'Achat', available_products=available_products, available_units=available_units)
...
def view_purchase(id):
    ...
    return render_template('purchases/view_purchase.html', title=f"Bon d'Achat {purchase.reference}", purchase=purchase)
...
def mark_as_paid(id):
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('purchases.view_purchase', id=id))
    return render_template('purchases/mark_paid.html', purchase=purchase, form=form, title=f'Marquer comme Payé - {purchase.reference}')
...
def edit_purchase(id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        if purchase.status == PurchaseStatus.RECEIVED:
            for item in purchase.items:
                if item.product:
                    old_stock_updates.append({'product': item.product, 'location': item.stock_location, 'quantity': float(item.quantity_ordered)})
                ...
                if product_id and quantity > 0 and price > 0:
                    ...
                    if unit_id:
                        try:
                            ...
                            pass
                    purchase_item = PurchaseItem(purchase_id=purchase.id, product_id=product_id, quantity_ordered=final_quantity, unit_price=final_unit_price, original_quantity=original_quantity, original_unit_id=original_unit_id, original_unit_price=original_unit_price, stock_location=stock_location, description_override=description_with_unit)
                    db.session.add(purchase_item)
                    items_added += 1
            except (ValueError, IndexError, TypeError) as e:
                print(f"Erreur traitement item {i}: {e}")
        ...
        if items_added == 0:
            ...
            available_products = Product.query.filter(Product.product_type.in_(['ingredient', 'consommable'])).all()
            available_units = Unit.query.filter_by(is_active=True).order_by(Unit.display_order).all()
            return render_template('purchases/edit_purchase.html', form=form, purchase=purchase, title='Modifier Bon d\'Achat', available_products=available_products, available_units=available_units)
        if purchase.status == PurchaseStatus.RECEIVED:
            for item in purchase.items:
                if item.product:
                    if item.stock_location == 'ingredients_magasin':
    ...
    available_products = Product.query.filter(Product.product_type.in_(['ingredient', 'consommable'])).all()
    available_units = Unit.query.filter_by(is_active=True).order_by(Unit.display_order).all()
    return render_template('purchases/edit_purchase.html', form=form, purchase=purchase, title=f'Modifier Bon d\'Achat {purchase.reference}', available_products=available_products, available_units=available_units)
...
def api_products_search():
    ...
    if len(search_term) < 2:
        return jsonify([])
    products = Product.query.filter(and_(Product.name.ilike(f'%{search_term}%'), Product.product_type.in_(['ingredient', 'consommable']))).limit(20).all()
    results = []
    for product in products:
        results.append({'id': product.id, 'name': product.name, 'unit': product.unit, 'cost_price': float(product.cost_price or 0), 'stock_magasin': product.stock_ingredients_magasin, 'stock_local': product.stock_ingredients_local})
...
def api_pending_count():
    ...
    count = Purchase.query.filter(Purchase.status.in_([PurchaseStatus.DRAFT, PurchaseStatus.REQUESTED])).count()
...
def api_product_units(product_id):
    ...
    results = []
    for unit in units:
        results.append({'id': unit.id, 'name': unit.name, 'base_unit': unit.base_unit, 'conversion_factor': float(unit.conversion_factor), 'unit_type': unit.unit_type})
    return jsonify(results)
